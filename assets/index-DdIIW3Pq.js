(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&o(l)}).observe(document,{childList:!0,subtree:!0});function s(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=s(n);fetch(n.href,a)}})();const w="song_mastery_data";class y{constructor(){this.songs=this._load()}_load(){try{const e=localStorage.getItem(w);return e?JSON.parse(e):[]}catch(e){return console.error("Failed to load data",e),[]}}_save(){try{localStorage.setItem(w,JSON.stringify(this.songs))}catch(e){console.error("Failed to save data",e)}}getAllSongs(){return this.songs}getSongs(e,s){return this.songs.filter(o=>o.instrument===e&&o.status===s)}getSong(e){return this.songs.find(s=>s.id===e)}addSong(e){return e.id||(e.id=crypto.randomUUID()),e.createdAt=Date.now(),this.songs.push(e),this._save(),e}updateSong(e,s){const o=this.songs.findIndex(n=>n.id===e);return o!==-1?(this.songs[o]={...this.songs[o],...s},this._save(),this.songs[o]):null}deleteSong(e){const s=this.songs.findIndex(o=>o.id===e);return s!==-1?(this.songs.splice(s,1),this._save(),!0):!1}getStats(e){const s=this.songs.filter(i=>i.instrument===e),o=s.length,n=s.filter(i=>i.status==="mastered").length,a=o-n,l=o===0?0:Math.round(n/o*100);return{total:o,mastered:n,toLearn:a,progress:l}}}const d=new y;class k{constructor(){this.app=document.getElementById("app"),this.currentView=null,this.root=document.createElement("div"),this.root.className="max-w-md mx-auto min-h-screen bg-stone-50 pb-24",this.app.appendChild(this.root)}render(e,s={}){this.root.innerHTML=e(s),this.bindEvents(),window.scrollTo(0,0)}bindEvents(){}navigateTo(e,s={}){window.dispatchEvent(new CustomEvent("navigate",{detail:{screen:e,params:s}}))}showConfirmModal(e,s){let o=document.getElementById("app-modal");o||(o=document.createElement("div"),o.id="app-modal",o.className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 hidden opacity-0 transition-opacity duration-200",document.body.appendChild(o)),o.innerHTML=`
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-100 flex justify-around items-center h-20 z-30 pb-2">
        ${[{id:"home",label:"Home",icon:p.home},{id:"guitar",label:"Guitar",icon:p.guitarNav},{id:"ukulele",label:"Ukulele",icon:p.ukuleleNav}].map(s=>{const n=t===s.id?"text-emerald-700":"text-stone-400 hover:text-stone-600";return`
    ...
    `,h={home:({guitarStats:t,ukuleleStats:e})=>`
        ...
        <div class="space-y-3">
            ...
            <div class="flex gap-2">
                ...
                <div class="flex-1 overflow-x-auto no-scrollbar flex gap-2 pb-1">
                    ${Object.keys(u).filter(l=>l!=="default").map(l=>{const i=u[l],r=i.color;return`
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1 min-w-0 pr-2">
                        ...
                        <div class="flex flex-wrap gap-1">
                            ${l.tags&&l.tags.length>0?l.tags.map(i=>`<span class="px-1.5 py-0.5 rounded text-[10px] font-bold border ${(u[i]||u.default).color}">${i}</span>`).join(""):'<span class="px-1.5 py-0.5 rounded text-[10px] font-medium border bg-stone-50 text-stone-300 border-stone-100">No Tags</span>'}
                        </div>
                    </div>
      ...
      <div class="p-6 space-y-6">
        <div>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
                ${t.tags.map(o=>{const n=u[o]||u.default;return`<span class="px-3 py-1 ${n.color} border text-xs font-bold rounded-full flex items-center gap-1">${n.icon} ${o}</span>`}).join("")}
            </div>
            `:""}
        </div>
      ...
      <form id="song-form" onsubmit="window.handleFormSubmit(event)" class="px-6 pt-6 space-y-6 flex-1 overflow-y-auto">
        ...
        <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2" id="tags-container">
                ${Object.keys(u).filter(c=>c!=="default").map(c=>{const x=(t.tags||[]).includes(c),b=u[c],g=b.color,v="bg-white text-stone-600 border-stone-200 hover:border-emerald-500";return`
                    <button type="button" 
                        onclick="this.classList.toggle('selected');
                                 if(this.classList.contains('selected')) {
                                     this.className = 'tag-select-btn px-4 py-2 rounded-full border text-sm font-bold transition-all active:scale-95 flex items-center gap-2 selected ${g}';
                                 } else {
                                     this.className = 'tag-select-btn px-4 py-2 rounded-full border text-sm font-medium transition-all active:scale-95 flex items-center gap-2 ${v}';
                                 }"
                        data-tag="${c}"
                        data-selected-class="${g}"
                        data-default-class="${v}"
                        class="tag-select-btn px-4 py-2 rounded-full border text-sm font-medium transition-all active:scale-95 flex items-center gap-2 ${x?`selected ${g} font-bold`:v}">
                        <span>${b.icon}</span>    
                        <span>${c}</span>
                    </button>
                    `}).join("")}
  ...
  `}};window.dispatchAppEvent=(t,e)=>{window.dispatchEvent(new CustomEvent(t,{detail:e}))};window.handleFormSubmit=t=>{t.preventDefault();const e=new FormData(t.target),s=Object.fromEntries(e.entries());s.capo&&s.capo.trim()!==""?s.capo=parseInt(s.capo):delete s.capo;const o=document.querySelectorAll(".tag-select-btn"),n=[];o.forEach(a=>{a.classList.contains("selected")&&n.push(a.getAttribute("data-tag"))}),s.tags=n,window.dispatchAppEvent("save-song",s)};window.handleFormChange=t=>{const e=document.getElementById("instrument-select"),s=document.getElementById("status-select"),o=document.getElementById("capo-field"),n=document.getElementById("chords-field");t.target===e&&(e.value==="guitar"||e.value==="ukulele"?o.style.display="block":o.style.display="none"),t.target===s&&(s.value==="mastered"?n.style.display="block":n.style.display="none")};window.currentFilters={query:"",capo:"",tag:""};window.applyFilters=()=>{const t=window.currentFilters.query.toLowerCase(),e=window.currentFilters.capo,s=window.currentFilters.tag;document.querySelectorAll(".song-item").forEach(o=>{const n=o.getAttribute("data-title"),a=o.getAttribute("data-artist"),l=o.getAttribute("data-capo")||"0",i=(o.getAttribute("data-tags")||"").split(",");let r=!0;t&&!n.includes(t)&&!a.includes(t)&&(r=!1),r&&e!==""&&(e==="0"?l!=="null"&&l!==""&&l!=="0"&&(r=!1):l!==e&&(r=!1)),r&&s!==""&&!i.includes(s)&&(r=!1),o.style.display=r?"block":"none"})};window.handleSearch=t=>{window.currentFilters.query=t.target.value,window.applyFilters()};window.handleFilterChange=(t,e,s)=>{const o=t==="tag",n=t,a=`${t}-filter-btn`;o&&window.currentFilters[n]===e?window.currentFilters[n]="":window.currentFilters[n]=e,document.querySelectorAll(`.${a}`).forEach(l=>{const i=l.getAttribute("data-default-class"),r=l.getAttribute("data-active-class"),c=l.getAttribute("data-value");window.currentFilters[n]===c?l.className=`${a} whitespace-nowrap text-xs font-bold rounded-lg px-3 py-2 transition-colors active:scale-95 flex items-center justify-center gap-1.5 ${r}`:l.className=`${a} whitespace-nowrap text-xs font-bold rounded-lg px-3 py-2 transition-colors active:scale-95 flex items-center justify-center gap-1.5 ${i}`}),window.applyFilters()};window.autoscrollState={interval:null,speed:1,isActive:!1};window.startAutoscroll=()=>{window.autoscrollState.interval&&clearInterval(window.autoscrollState.interval),window.autoscrollState.isActive=!0;const t=document.getElementById("icon-play"),e=document.getElementById("icon-pause");t&&t.classList.add("hidden"),e&&e.classList.remove("hidden");const s=()=>{window.scrollBy(0,1),window.innerHeight+window.scrollY>=document.body.offsetHeight&&window.stopAutoscroll()},n=50/window.autoscrollState.speed;window.autoscrollState.interval=setInterval(s,n)};window.stopAutoscroll=()=>{window.autoscrollState.interval&&clearInterval(window.autoscrollState.interval),window.autoscrollState.interval=null,window.autoscrollState.isActive=!1;const t=document.getElementById("icon-play"),e=document.getElementById("icon-pause");t&&t.classList.remove("hidden"),e&&e.classList.add("hidden")};window.adjustSpeed=t=>{let e=window.autoscrollState.speed+t;e=Math.round(e*10)/10,e<.1&&(e=.1),e>10&&(e=10),window.autoscrollState.speed=e;const s=document.getElementById("speed-display");s&&(s.textContent=e.toFixed(1)+"x"),window.autoscrollState.isActive&&window.startAutoscroll()};class ${constructor(){this.init()}init(){window.addEventListener("navigate",e=>this.route(e.detail.screen,e.detail.params)),window.addEventListener("popstate",e=>{e.state?this.render(e.state.screen,e.state.params,!1):this.route("home",{},!1)}),window.addEventListener("save-song",e=>{const s=e.detail;s.id?d.updateSong(s.id,s):d.addSong(s),window.history.back()}),window.addEventListener("delete-song",e=>{m.showConfirmModal("Are you sure you want to delete this song? This action cannot be undone.",()=>{d.deleteSong(e.detail.id),window.history.back()})}),window.addEventListener("mark-mastered",e=>{d.updateSong(e.detail.id,{status:"mastered"}),this.route("detail",{id:e.detail.id},!0)}),window.addEventListener("random-song",e=>{const s=e.detail.instrument||"guitar";d.getSongs(s,"mastered");const o=d.getSongs(s,"pool"),n=d.getSongs(s,"mastered"),a=[...o,...n];if(a.length>0){const l=a[Math.floor(Math.random()*a.length)];this.route("detail",{id:l.id})}else alert(`No ${s} songs found! Add some first.`)}),window.addEventListener("toggle-autoscroll-panel",()=>{const e=document.getElementById("autoscroll-panel");e&&(e.classList.contains("hidden")?(e.classList.remove("hidden"),requestAnimationFrame(()=>{e.classList.remove("opacity-0","scale-95")})):(e.classList.add("opacity-0","scale-95"),setTimeout(()=>{e.classList.add("hidden")},200)))}),window.addEventListener("toggle-autoscroll-state",()=>{window.autoscrollState.isActive?window.stopAutoscroll():window.startAutoscroll()}),window.addEventListener("adjust-speed",e=>{window.adjustSpeed(e.detail.delta)}),this.route("home",{},!0)}route(e,s={},o=!1){window.stopAutoscroll&&window.stopAutoscroll(),o?window.history.replaceState({screen:e,params:s},"","#"+e):window.history.pushState({screen:e,params:s},"","#"+e),this.render(e,s)}render(e,s){switch(e){case"home":const o=d.getStats("guitar"),n=d.getStats("ukulele");m.render(h.home,{guitarStats:o,ukuleleStats:n});break;case"list":const a=d.getSongs(s.instrument,s.status||"pool");a.sort((r,c)=>c.createdAt-r.createdAt),m.render(h.list,{instrument:s.instrument,status:s.status,songs:a});break;case"detail":const l=d.getSong(s.id);if(!l){this.route("home",{},!0);return}m.render(h.detail,{song:l});break;case"form":const i=s.id?d.getSong(s.id):{};m.render(h.form,{song:i,instrument:s.instrument,status:s.status});break;default:m.render(h.home)}}}new $;
